...
public class MybatisTests {
	...
	@Test
	public void MemoMapperTests() throws Exception {
		memoMapper.Insert(new MemoDto(123, "TEST"));
		memoMapper.Insert(new MemoDto(124, "TEST"));
		memoMapper.Insert(new MemoDto(125, "TEST"));
		memoMapper.Update(new MemoDto(123,"ABC"));
		memoMapper.Delete(123);
		memoMapper.Delete(124);
		memoMapper.Delete(125);
		...
		result.forEach((map)->{
			for(String key : map.keySet()) {
				log.info(key+" : " + map.get(key));		// map.get() : Returns the value to which the specified key is mapped
			}
