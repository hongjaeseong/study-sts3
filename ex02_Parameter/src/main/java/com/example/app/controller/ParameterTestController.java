...
public class ParameterTestController {
	...
	@GetMapping("/p02")
	public void p2(@RequestParam(required = false, name="username") String name) {	//파라미터값 여부 선택 가능
	...
	public void p3(@RequestParam(required = true, name="username") String username) {	//파라미터값 필수 입력
		log.info("POST /test/p03..." + username);
	}
	
	// @RequestParam	: 동기요청 방식의 기본 / form 기반 요청 받기
	// @RequestBody		: 비동기요청 방식의 기본 / form / json 등을 파라미터 값으로 받기
	...
	public void p5(@RequestParam(required=false,defaultValue="홍재성") String username) {
	...
	/*
	 * @GetMapping("/forward1") public String f1(Model model) {
	 * log.info("GET /test/forward1...");
	 * model.addAttribute("forward1","forward1Value"); return
	 * "forward:/test/forward2"; }
	 * 
	 * @GetMapping("/forward2") public String f2(Model model) {
	 * log.info("GET /test/forward2...");
	 * model.addAttribute("forward2","forward2Value"); return
	 * "forward:/test/forward3"; }
	 * 
	 * @GetMapping("/forward3") public void f3(Model model) {
	 * log.info("GET /test/forward3...");
	 * model.addAttribute("forward3","forward3Value"); }
	 */
	
	//Redirect (redirect1 -> redirect2 -> redirect3)
	//새로운 request 객체를 사용자에게 새로 요청함
	...
	public String r1(RedirectAttributes redirectAttributes) {
		...
		// ?redirect2=redirect2Value : 이 부분이 쿼리 스트링
